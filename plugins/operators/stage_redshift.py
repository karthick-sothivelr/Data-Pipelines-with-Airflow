...
class StageToRedshiftOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        if self.truncate:
            self.log.info(f'Truncate Redshift table {self.target_table}')
        ...
        formatted_sql = StageToRedshiftOperator.copy_sql.format(
            self.target_table, s3_path, credentials.access_key,
            credentials.secret_key, self.data_format, self.region,
        )

        # Run query
        self.log.info('Copy data from {s3_path} to Redshift table {self.target_table}')
